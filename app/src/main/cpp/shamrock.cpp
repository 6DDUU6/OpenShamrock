...
Java_moe_qiufuluo_shamrock_xposed_hooks_PullConfig_testNativeLibrary(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF("加载Shamrock库成功~");
}

extern "C"
JNIEXPORT jstring JNICALL
Java_moe_qiufuluo_shamrock_utils_MD5_genFileMd5Hex(JNIEnv *env, jobject thiz, jstring file_path) {
...
Java_moe_qiufuluo_shamrock_utils_MD5_genFileMd5(JNIEnv *env, jobject thiz, jstring file_path) {
    auto cPathStr = env->GetStringUTFChars(file_path, nullptr);
    std::filesystem::path filePath(cPathStr);
    if (!std::filesystem::exists(filePath)) {
        jclass exClass = env->FindClass("java/io/FileNotFoundException");
...
extern "C"
JNIEXPORT jstring JNICALL
Java_moe_qiufuluo_shamrock_utils_MD5_getMd5Hex(JNIEnv *env, jobject thiz, jbyteArray bytes) {
